...
struct ModalPreview<VM: ModalVM, Pickers: View>: View {
  enum ModalBody {
    case shortText
    case longText
  }
...
"""
  ...
  private let presentController: (UKModalController.Content?, UKModalController.Content, UKModalController.Content?) -> Void
  @ViewBuilder private let additionalPickers: () -> Pickers?

  init(
    model: Binding<VM>,
    presentController: @escaping (
      _ header: UKModalController.Content?,
      _ body: UKModalController.Content,
      _ footer: UKModalController.Content?
    ) -> Void,
    @ViewBuilder additionalPickers: @escaping () -> Pickers?
  ) {
    self._model = model
    self.presentController = presentController
    self.additionalPickers = additionalPickers
  }
        ...
        UKComponentPreview(model: .init { $0.title = "Show Modal" }) {
          UKButton {
            self.presentController(
              ...
              self.modalFooter.map { footer in
                return { dismiss in
